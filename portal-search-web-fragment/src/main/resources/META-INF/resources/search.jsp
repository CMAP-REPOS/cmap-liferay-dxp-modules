...
<aui:row
	cssClass='<%="search-layout"
						+ (searchDisplayContext.isShowMenu() ? " clearfix menu-column" : StringPool.BLANK)%>'>
	<aui:col span="<%=6%>">
		<aui:form action="<%=searchURL%>" method="get" name="fm"
			onSubmit="event.preventDefault();">
			<liferay-portlet:renderURLParams varImpl="searchURL" />
			<aui:input name="<%=SearchContainer.DEFAULT_CUR_PARAM%>"
				type="hidden"
				value="<%=ParamUtil.getInteger(request, SearchContainer.DEFAULT_CUR_PARAM,
	...
	<aui:col id="facetNavigation" span="<%=6%>">

		<%
			for (SearchFacet searchFacet : ListUtil.sort(searchDisplayContext.getEnabledSearchFacets(),
							SearchFacetComparator.INSTANCE)) {
						...
						searchFacet.includeView(request,
	...
	<aui:col span="<%=12%>">
		...
		<c:if test="<%=searchDisplayContext.isDisplayOpenSearchResults()%>">
			<liferay-ui:panel collapsible="<%=true%>"
				cssClass="open-search-panel" extended="<%=true%>"
				id="searchOpenSearchPanelContainer" persistState="<%=true%>"
				title="open-search">
...
<%
	...
	if (Validator.isNotNull(searchDisplayContext.getKeywords())) {
		...
		if (StringUtil.startsWith(pageKeywords, Field.ASSET_TAG_NAMES + StringPool.COLON)) {
			pageKeywords = StringUtil.replace(pageKeywords, Field.ASSET_TAG_NAMES + StringPool.COLON,
					StringPool.BLANK);
		}
	}
